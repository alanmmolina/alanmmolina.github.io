...
Let me show you what I mean. You've got a pipeline dumping sales data into `s3://my-bucket/sales/` every hour. Each run creates a new Parquet file. Life is good. Then someone asks for last month's revenue, and you realize one of those hourly runs wrote garbage. Half the rows have negative prices. You need to roll back, but there is no "roll back." There is only "find the bad file and hope you can rebuild the state without it." Your Parquet files have no memory. They have no snapshot to return to.

Or maybe you added a column to the upstream system. Your pipeline writes files with the new schema, but your downstream readers were compiled against the old one. They crash. Parquet files don't enforce a schema across the table. Each file is its own little island, and nothing checks whether the islands agree.

Or two jobs run at the same time and both write to the same partition. One overwrites the other's files. No error gets raised, no warning is logged. The data is simply gone. Parquet files don't coordinate. They don't even know the other write happened.

These are not edge cases. They are the default experience of using files as tables. And for a long time, the answer was "be careful": careful scheduling, careful naming, careful manual cleanup. Careful is not a protocol.

Delta Lake was built to solve exactly this. It wraps a directory of Parquet files in a transaction log. That log is the difference between "here are some files, good luck" and "here is a table, here is its history, here is what changed and when."

> [!info] What files can't do
> The three things Parquet files alone can't give you: **memory** (no snapshots, no rollback), **rules** (no schema enforcement across files), and **coordination** (no protection against concurrent writes corrupting each other). A table needs all three. A directory of files provides none.
...
In 2019, Databricks open-sourced the project through the [Linux Foundation](https://www.linuxfoundation.org/). This matters. A table format that lives inside a single vendor's platform is a feature. A table format governed by an open community with an [Apache 2.0 license](https://www.apache.org/licenses/LICENSE-2.0) is infrastructure. Today, Delta Lake lives under the [`delta-io` GitHub organization](https://github.com/delta-io/delta) with over 190 contributors from more than 70 organizations, including Adobe, Amazon, Apple, Microsoft, and others who have no interest in letting one company own the format their data lives in. The project is currently at version 4.3.0 and supports query engines across the board: Spark, Trino, Presto, Flink, DuckDB, Snowflake, BigQuery, Athena, Redshift. You can write with one engine and read with another. The format is the contract.
...
- **Catalog:** A registry that maps a table name like `sales.orders` to a path like `s3://my-bucket/orders/`. The catalog tells you where to look. The log tells you what you'll find there.
...
> [!tip] Your data stays portable
> This is worth underlining: Delta Lake does not invent a new file format. The data files are standard Parquet, readable by Pandas, DuckDB, or any other Parquet-compatible tool. The Delta-specific part is the `_delta_log/` directory sitting next to them. That separation means your data is never locked into a proprietary format.
...
```json
{"metaData":{"id":"af23c9d7-fff1-4a5a-a2c8-55c59bd782aa","format":{"provider":"parquet","options":{}},"schemaString":"{\"type\":\"struct\",\"fields\":[{\"name\":\"id\",\"type\":\"integer\",\"nullable\":true},{\"name\":\"name\",\"type\":\"string\",\"nullable\":true}]}","partitionColumns":[],"configuration":{},"createdTime":1720000000000}}
{"protocol":{"minReaderVersion":1,"minWriterVersion":2}}
{"add":{"path":"part-00000-abc.c000.snappy.parquet","partitionValues":{},"size":4523,"modificationTime":1720000000000,"dataChange":true,"stats":"{\"numRecords\":100,\"minValues\":{\"id\":1,\"name\":\"Alice\"},\"maxValues\":{\"id\":100,\"name\":\"Zoe\"}}"}}
```
...
```json
{"remove":{"path":"part-00000-abc.c000.snappy.parquet","deletionTimestamp":1720000003600,"dataChange":true}}
{"add":{"path":"part-00000-def.c000.snappy.parquet","partitionValues":{},"size":4200,"modificationTime":1720000003600,"dataChange":true,"stats":"{\"numRecords\":99,\"minValues\":{\"id\":1,\"name\":\"Alice\"},\"maxValues\":{\"id\":100,\"name\":\"Zoe\"}}"}}
```
...
### The Catalog Layer: From Name to Path

So far we have assumed you already know where your table lives. But how does `spark.read.table("sales.orders")` turn into `s3://my-bucket/orders/`?

That's the catalog. In most setups, it's the Hive metastore, a small database that maps table names to paths and stores a cached copy of the schema for quick lookup. Spark's default catalog is the Hive metastore. Databricks uses Unity Catalog. AWS Glue, Polaris, and others do the same job.
...
```sql
SELECT * FROM sales.orders WHERE order_date = '2024-06-15'
```

```mermaid
flowchart TD
    Q[SELECT query] --> C[Catalog: resolve table name to path]
    C --> LC[Read _last_checkpoint]
    LC --> CP[Load checkpoint Parquet]
    CP --> RJ[Replay recent JSON commits]
    RJ --> S[Snapshot: list of data files with stats]
    S --> PP[Partition pruning: skip irrelevant directories]
    PP --> FP[File pruning: skip files using min/max stats]
    FP --> R[Read remaining Parquet files]
    R --> RES[Return results]
```

First, the engine asks the catalog: "where does `sales.orders` live?" The catalog returns `s3://my-bucket/orders/`. The engine now knows which directory to inspect.
...
Next, the engine prunes. The query filters on `order_date = '2024-06-15'`, and the partition column is `order_date`. The engine looks at the partition values recorded for each file in the snapshot. Files in `order_date=2024-06-15/` are relevant. Files in `order_date=2024-06-14/` are skipped. Their paths aren't even opened.

Even within the relevant partition, the per-file statistics help. If the query also had a filter like `AND amount > 1000`, and a file's `amount` column stats say `min: 5, max: 50`, that file gets skipped. No row in it can satisfy the predicate. This is file-level pruning, and it costs nothing beyond reading a few JSON fields from the log.
...
> [!note] Soft deletes
> If the table uses deletion vectors, the reader also loads the relevant DV files and applies them during the scan, skipping the soft-deleted rows without anyone having rewritten the data files.

---

## The Write Path

Writes follow the same pattern with one critical addition: the commit itself is a race.

Let's trace an INSERT. You run:
...
```mermaid
flowchart TD
    R[Read current snapshot] --> W[Write new Parquet files]
    W --> C[Attempt atomic commit]
    C --> S{Version claimed?}
    S -->|No| OK[Commit succeeds. Readers see new data.]
    S -->|Yes| FAIL[Conflict detected. Raise ConcurrentAppendException.]
    FAIL --> RETRY[Retry: read new snapshot, re-write if needed, commit at next version.]
    RETRY --> C
```
...
Now comes the commit. The engine constructs a new JSON commit file containing an `add` action for each new file. It writes this file to `_delta_log/_staged_commits/00000000000000000145.<uuid>.json` and then attempts to make it visible by placing it at `_delta_log/00000000000000000145.json`.

This is where the storage system earns its keep. The commit must be atomic. The file either appears in its entirety or does not appear at all.[^3]
...
If version 145 is still unclaimed when the engine attempts the commit, the file lands and the write succeeds. The engine just created a new atomic version of the table. Every future reader will see this data.

If another writer claimed version 145 first, the engine's commit attempt fails. It detects the conflict (the version number is already taken) and raises a `ConcurrentAppendException`. The operation fails rather than silently corrupting the table. The engine can retry: read the new snapshot (which now includes the other writer's changes), re-write its files if needed, and try committing at version 146.
